#include <help/ecg_memory.h>

namespace ecg {
	ecg_mem_handler::ecg_mem_handler(const ecg_mem_init_info_t& init_info) {
		m_ecg_memory_size = init_info.total_bytes;
		m_is_array = init_info.is_array;
		m_memory_type = init_info.type;
		m_ecg_memory = init_info.ptr;
		
		if (++m_ecg_global_handler_id == 0) ++m_ecg_global_handler_id;
		m_ecg_handler_id = m_ecg_global_handler_id;
	}
	
	bool ecg_mem_handler::is_array() const { return m_is_array; }
	uint64_t ecg_mem_handler::get_memory_handler() const { return m_ecg_handler_id; }

	std::shared_ptr<void> ecg_mem_handler::get_pointer() const { return m_ecg_memory; }
	ecg_memory_type ecg_mem_handler::get_memory_type() const { return m_memory_type; }
	size_t ecg_mem_handler::get_memory_size() const { return m_ecg_memory_size; }

	bool ecg_mem_handler::operator==(const ecg_mem_handler& other) const {
		return 
			m_ecg_memory == other.m_ecg_memory && 
			m_ecg_handler_id == other.m_ecg_handler_id;
	}

	bool ecg_mem_handler::operator!=(const ecg_mem_handler& other) const {
		return !(*this == other);
	}

	ecg_mem_ctrl& ecg_mem_ctrl::get_instance() {
		static ecg_mem_ctrl instance;
		return instance;
	}

	bool ecg_mem_ctrl::is_contains_handler(uint64_t handler_id) {
		auto it = m_allocated_objects.find(handler_id);
		if (it != m_allocated_objects.end()) return true;
		return false;
	}

	ecg_mem_handler ecg_mem_ctrl::ecg_malloc(const ecg_mem_init_info_t& init_info) {
		ecg_mem_handler handler(init_info);
		uint64_t handler_id = handler.get_memory_handler();
		m_allocated_objects.try_emplace(handler_id, handler);
		return handler;
	}
	
	bool ecg_mem_ctrl::ecg_free(const ecg_mem_handler& handler) {
		uint64_t handler_id = handler.get_memory_handler();
		return ecg_free(handler_id);
	}

	bool ecg_mem_ctrl::ecg_free(uint64_t handler_id) {
		auto it = m_allocated_objects.find(handler_id);
		if (it != m_allocated_objects.end()) {
			m_allocated_objects.erase(it);
			return true;
		}

		return false;
	}
}